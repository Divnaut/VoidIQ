...
const uint8_t transmit_address[]={0x5C, 0x01, 0x3B, 0x96, 0xB1, 0x8C}; //Reciever address: Towards gateway node
...
void setup() {
  ...
  // int32_t gateway_channel = get_wifi_channel(WIFI_SSID);
  // Serial.printf("[Master] Detected Gateway Channel: %d\n", gateway_channel);
  // if (esp_now_init() != ESP_OK) {
  //   Serial.println("Error initializing ESP-NOW");
  //   //return;
  // }

  //switching to the gateway's channel:
  //esp_wifi_set_promiscuous(true);
  //set 2 as the default wifi channel for all communications
  //esp_wifi_set_promiscuous(false);
  // Serial.printf("[Master] Switched to Channel: %d\n", gateway_channel);
  esp_now_register_recv_cb(on_recv_callback);
  // memcpy(peerInfo.peer_addr, transmit_address, 6);
  // peerInfo.channel = 0; // using default channel
  ...
  //will need to use a mutex: configure channel 2 and do not switch until transmission is complete
  // }
